...
previews would need SSR/prerender — future work). `robots.txt` + `sitemap.xml` are
generated by a small Vite plugin (`vite.config.ts`) from `VITE_SITE_URL` (so the
sitemap's absolute `<loc>`s and the `Sitemap:` line track the deploy origin), and
served in dev/preview too. Auth + signed-in pages are `noindex` and `Disallow`ed.
...
- **Web:** `VITE_API_URL` (default empty → relative `/api`, via the dev proxy).
  ...
  **build time** for the absolute URLs in `sitemap.xml` / `robots.txt`; canonical
  and OG URLs are resolved at runtime from the live origin, so only the sitemap
  needs it. Set it in production CI.
