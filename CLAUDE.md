...
The **auth foundation** (**email-first registration** / login / session, plus **email
verification and password reset** — transactional mail via Resend, a new account
finished via an emailed completion link that sets its first password and verifies the
address — hardened with **Cloudflare Turnstile CAPTCHA and per-IP
...
A parallel per-user **wish list** tracks the cards a user *wants to buy* — the same
holding shape, per-set views, values, quick-add, and count editing as the collection
(the UI pieces are `list`-parameterised rather than duplicated), minus the whole
import/sync layer. It differs where wanting differs from owning: the per-game wish-list
landing defaults to just the sets you've listed in, with a segmented toggle
(`?sets=all`) flipping to **every** catalog set (like the cards menu); its browse
views default ghost mode **off** like the collection (`?ghosts=1` opts in — the full
catalog in scope with un-wishlisted cards dimmed and add-in-place controls that write
to the wish list); and its pages show only the **total** value (no bulk slice — a wish
list is a shopping list, so only what it costs matters).
...
Each route mirrors its collection twin exactly (params, ordering, errors, caps):
...
stores/auth.ts     Pinia store: in-memory accessToken + user, isAuthenticated, login/logout/refresh/fetchMe/tryRestore/setSession + authFetch helper (deliberately NO register action — email-first registration mints no session, so RegisterView calls the api fn directly; completing registration DOES sign in, so CompleteRegistrationView adopts the session via setSession)
