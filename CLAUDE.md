...
scryfall/          MTG provider (the first game)
  ...
  search/          Scryfall-style query compiler: lexer.rs → parser.rs (recursive descent: and/or/-/parens/quotes → Node AST) → compile/ (one submodule per filter family → sea_orm::Condition); error.rs (SearchError → 422); values always parameterised
...
lib/…              small pure helpers: cardPrice (tile price pick, foil fallback), cardSort (sort options ↔ API sort/dir), cardSize (grid density), ownership (owned-count labels), importSummary (import-result lines), persistedRef (localStorage-backed ref)
