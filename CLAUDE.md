...
landing lists **every** catalog set (like the cards menu, not just sets you've listed
in), and its browse views default ghost mode **on** (`?ghosts=0` to see only listed
cards) so the full catalog renders with un-wishlisted cards dimmed and add-in-place
controls that write to the wish list.
