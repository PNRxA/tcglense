...
(`?sets=all`) flipping to **every** catalog set (like the cards menu); its browse views
default ghost mode **on** (`?ghosts=0` to see only listed cards) so the full catalog
renders with un-wishlisted cards dimmed and add-in-place controls that write to the
wish list; and its pages show only the **total** value (no bulk slice — a wish list is
a shopping list, so only what it costs matters).
