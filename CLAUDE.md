...
(`?sets=all`) flipping to **every** catalog set (like the cards menu); its all-cards
browse view defaults ghost mode **on** (`?ghosts=0` to see only listed cards) so the
full catalog renders with un-wishlisted cards dimmed and add-in-place controls that
write to the wish list, while its set-scoped view defaults ghosts **off** like the
collection (`?ghosts=1` opts in); and its pages show only the **total** value (no bulk
slice — a wish list is a shopping list, so only what it costs matters).
...
security_tests/    HTTP-level security tests driving the real build_router in-process (tower oneshot): refresh rotation/reuse, generic login failures, email verification + password reset (via the harness's capturing mailbox), cookie/CORS/caching contracts, request bodies, search, collection + import, wishlist (incl. wishlist↔collection independence)
