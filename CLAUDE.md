...
or view every owned card (`/collection/{game}/cards`). Either card grid can optionally
**show "ghosts"** of the cards in scope you *don't* own — a per-view toggle (`?ghosts=1`,
default off) that swaps the owned-only list for the public catalog list with unowned cards
dimmed, so a set's gaps (and a "X of Y owned" completion count) read at a glance and can be
quick-added in place. A collection can also be
