...
| `GET /api/games/{game}/search?q&limit` | `SearchResults` — **one query across the catalog** (the homepage search box's read): `{ cards, products, precons, keywords }`, each a `SearchGroup<T> = { data: T[], has_more }` of the top `limit` (default 5, max 10) matches of that kind — `Card` rows (**one per distinct name**, a representative printing each), and `Product` / `PreconDeck` / `KeywordEntry` rows unchanged from their own listings, so a client renders a hit with the tile it already has. Every leg applies the sealed/precon lists' name rule — each whitespace-separated word an order-independent, case-insensitive name substring — **never** the Scryfall grammar (a colon in a card name must not `422` every group at once; the full grammar is the card listing's), and leads each group with names that *start* with `q`. Keywords match by name only, never reminder text. `has_more` comes from one row of over-fetch, never a `COUNT(*)`. The same for every visitor (the SPA adds the caller's own decks client-side from the deck list), so CDN/ETag-cached and per-IP limited like the autocomplete. Blank `q` = empty groups; `404` unknown game; `422` over 32 words |
