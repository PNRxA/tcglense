...
| `GET /api/games/{game}/sets/{code}/drops?q&page&page_size` | a drop-grouped set's cards broken into **Secret Lair drops** (Scryfall's curated drop titles), **paginated by drop** — `{ data: DropGroup[], page, page_size, total, has_more }` where `DropGroup = { slug, title, card_count, cheapest_singles_usd, cards: Card[] }` and `total` counts drops. `cheapest_singles_usd` is the drop's "cheapest singles" total — the sum, over its cards, of each card's cheaper finish (regular vs foil) as a canonical USD decimal string (the SPA renders it in the display currency), or `null` when nothing in the drop is priced; its scope matches `card_count`. Drops keep Scryfall's order; within a drop, cards are by collector number. Cards not in the snapshot fall into a trailing `"Other"` group (`slug: null`). `404` if the set isn't drop-grouped (use `has_drops`); optional `q` filters cards, dropping now-empty drops |
...
| `GET /api/collection/{game}/movers` | — | `CollectionMovers` keeps the card series and a parallel `sealed` series with the same windows — each contains its own five largest holding-value gainers/losers for 1d / 7d / 30d / 1y / 2y / 3y / all captured history (see below). An empty latest-day comparison retries from the previous available snapshot. No query parameters. |
