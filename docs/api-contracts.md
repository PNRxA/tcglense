...
| `GET /api/alerts/channels` | — | `AlertChannels { discord_webhook_url, telegram_bot_token, telegram_chat_id, email_enabled, email_available }` — the caller's delivery settings (empty defaults if never set). `email_available` = `ALERTS_EMAIL_ENABLED` **and** an email provider is configured. Returned to the owner to prefill the settings form |
| `PUT /api/alerts/channels` | `{ discord_webhook_url?, telegram_bot_token?, telegram_chat_id?, email_enabled }` | `AlertChannels` — replace the settings (a blank string clears a field). The Discord webhook URL is **host-allow-listed** to a `discord.com` webhook (`422` otherwise — SSRF gate); Telegram needs both the token and the chat id, or neither (`422`) |
