...
| `GET /api/games/{game}/precons/groups?group&page&page_size&q&set&type&sort` | `Page<PreconGroup>` — the same decks bucketed and **paginated by group**, so a group is never split across a boundary (the precon mirror of `/sets/{code}/drops`). `group=set` (default) buckets by the publishing set, newest **set** first — a set's date is the catalog's, not its decks', so a Secret Lair deck released years after the `sld` set still sits with `sld`. `group=type` buckets by deck category, **biggest first** (the facets dropdown's own order), which is what makes a 70-deck set readable — Marvel ships 51 Jumpstart themes beside 12 Box Sets, and 136 of 295 sets span more than one type. `sort=name` orders either grouping by heading. `PreconGroup = { slug, title, released_at, set_code, deck_count, decks }` — the drop/sub-type group shape, so one client component renders either. `decks` is a **preview capped at 24**, while `deck_count` is the group's real size: a group is otherwise unbounded (`group=type` puts 570 decks in `Jumpstart` alone, and an uncapped page answered ~788 KB, buffered whole to compute its `ETag`), so `deck_count > decks.length` means truncated — link out to the flat listing with the group's own filter, which is never capped. Shares the flat list's filter builder, so a filter selects exactly the same decks in every view (a test pins that; only the order may differ) |
