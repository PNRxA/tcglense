...
| `GET /api/ready` | — | `200 { status: "ready" }` after a database round-trip | `503 { status: "unavailable" }` without internal details when storage is unavailable · `503 { status: "maintenance" }` when `MAINTENANCE_MODE=true` · `503 { status: "starting" }` while the boot migrations are still running (the listener binds first so `/api/health` stays 200; app traffic gets a non-cacheable `503 { code: "starting" }` until the schema is ready) |
