...
| `POST /api/auth/refresh` | — (refresh cookie) | `200 { access_token }` + **rotated** cookie | `401` if missing/invalid/expired/reuse-burned (clears cookie); `401` **without** touching the cookie for a benign concurrent double-submit (`"refresh token superseded"` — the SPA retries once to pick up the winner's cookie) |
