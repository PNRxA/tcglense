...
use crate::handlers::shared::{
    CardResponse, CopyFilter, HoldingSummaryRow, Page, SortDir, apply_card_sort, build_page,
    copies_expr, dedupe_ids, load_card, narrow_summary_rows, require_game, resolve_holdings_list,
...
#[utoipa::path(
    ...
    params(
        ...
        ("min_copies" = Option<i32>, Query, description = "Keep only cards held in at least this many copies (of the `finish` counter)"),
        ("max_copies" = Option<i32>, Query, description = "Keep only cards held in at most this many copies (of the `finish` counter)"),
        ("finish" = Option<String>, Query, description = "Which counter the copy bounds read: `any` (regular + foil, default), `regular`, or `foil` — the latter two also require at least one copy of that finish"),
    ),
    responses(
...
/// `collection_items` columns, so nothing is ambiguous across the join (both tables carry
/// a `game` column).
