...
pub(super) fn is_predicate(
    ...
    let positive: Condition = match v.as_str() {
        ...
        //
        // `foil` also matches a nonfoil-only printing whose foil is a separately-modelled
        // `…★` Scryfall object folded onto it (`sld` `1587` ← `1587★`). Those bases really
        // are obtainable in foil — that's why the catalog lists them once, with the star's
        // foil price on them — but their own `finishes` stays `nonfoil`, because
        // nonfoil-**only** is the load-bearing half of the pairing rule everywhere it
        // lives (`scryfall::foil_variants`). Without this arm the fold would quietly empty
