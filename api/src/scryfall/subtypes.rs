...
//! drop snapshot; an override wins over the derived classification. The shipped file
//! curates MTG's panoramic Borderless Scene runs (issue #315) — the one treatment
//! Scryfall's bulk data doesn't mark — and is regenerated by
//! `api/scripts/gen-subtype-overrides.mjs`.
...
/// special cards are curated still offers the by-treatment view. The shipped file touches
/// MTG's Borderless Scene sets (issue #315).
...
mod tests {
    ...
    fn overrides_parse() {
        ...
        // and is populated with the curated Borderless Scene runs (issue #315). Every entry
        // landing means its `subtype` slug is known — `build_overrides` drops unknown ones.
        let table = build_overrides(OVERRIDES_JSON);
        assert!(!table.is_empty(), "the curated Borderless Scene snapshot is populated");
        assert!(
            table.values().all(|s| s.slug == "borderless-scene"),
            "every shipped override targets the borderless-scene sub-type",
        );
        // Spot-check known scene cards (Assassin's Creed Florence panorama, LOTR Shire party).
        for (set, cn) in [("acr", "111"), ("ltr", "399")] {
            assert_eq!(
                table.get(&("mtg".into(), set.into(), cn.into())).map(|s| s.slug),
                Some("borderless-scene"),
                "{set}/{cn} is a curated Borderless Scene card",
            );
        }
    }
