...
pub(super) enum UserRoute {
    ...
    Analytics,
    /// The expensive collection/deck import and file/paste-upload endpoints, which do
    /// real server-side work (an upstream fetch, a parse of untrusted content, or
    /// database writes). A much tighter cap.
    Import,
}
...
impl UserRoute {
    ...
    pub(super) fn from_path(path: &str) -> Self {
        ...
        // the same uncapped whole-holdings drain / scan, so the same tighter bucket. Its
        // shopping list (`buy-list`, issue #292) is the same shape one size down — a
        // `COUNT(*)` over the whole filtered wish list plus a 500-row projection, and a
        // product page besides — so it rides the same bucket rather than the browse budget.
        if let Some(rest) = path.strip_prefix("/api/wishlist/")
            && let Some((_game, "cards/export" | "breakdown" | "buy-list")) = rest.split_once('/')
        {
            return Self::Analytics;
        }
            ...
            && let Some((_game, tail)) = rest.split_once('/')
        {
            ...
            // deck, and the suggestions read (issue #684), which scans the caller's whole
            // collection. Cheap for a real deck, but the same "whole-holdings scan per
            // request" shape as the collection analytics above, so the same tighter bucket
            // rather than the generous General one.
            if matches!(
                tail.rsplit('/').next(),
                Some(
                    "stats"
