...
// Each by-drop header shows the drop's "cheapest singles" total — the sum of every card's
// cheaper finish, computed server-side (DropGroup.cheapest_singles_usd) — rendered in the
// viewer's display currency (null when nothing in the drop is priced). Read straight off the
// drops query (only drops carry the field), which in the by-drop view *is* `groups`, so the
// array aligns index-for-index with the section v-for below (#456).
const money = useCurrency()
const singlesTotals = computed(() =>
  (dropsQuery.data.value?.data ?? []).map((drop) => money.formatUsd(drop.cheapest_singles_usd)),
)
        ...
        <template v-if="grouped">
          ...
          <UpdatingOverlay :loading="groupLoading">
            <DropSection
              ...
              :drop="cardGroup"
            >
              <!-- Drops only: the cheapest-singles total, right-aligned in the header.
                   Omitted for sub-type groups and for drops with nothing priced. -->
              <template v-if="byDrop && singlesTotals[i]" #meta>
                <span class="text-muted-foreground text-sm font-normal">cheapest singles</span>
