...
import { GLOSSARY_GAME, keywordSlug, relatedKeywords } from '@/lib/keywords'
...
export function useKeywordsQuery() {
  return useQuery({
    queryKey: ['keywords', GLOSSARY_GAME],
    queryFn: () => getKeywords(GLOSSARY_GAME).then((response) => response.data),
    staleTime: Infinity,
  })
}
...
export function useKeywordEntry(slug: Ref<string>) {
  const query = useKeywordsQuery()
  const entries = computed<KeywordEntry[]>(() => query.data.value ?? [])
