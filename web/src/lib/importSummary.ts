...
/**
 * Human-readable summary lines for a completed collection import, for the import dialog's
 * result panel. Kept as a pure function (no Vue) so it's unit-testable and could be reused
 * by any other import-outcome surface.
 *
 * Lead line: how many cards were imported (and total copies). Then any unmatched (skipped)
 * cards; then any removed to mirror the list. Counts are localized.
 */
export function formatImportSummaryLines(summary: ImportSummary): string[] {
  ...
  lines.push(
    `Imported ${summary.matched_cards.toLocaleString()} card${summary.matched_cards === 1 ? '' : 's'} ` +
