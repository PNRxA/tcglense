...
// The detail-page card sub-sections — a sealed product's "Cards in this product" and a card's
// "Other printings" — render one size larger than the catalog default at every step: both live
// in a narrower main column and list far fewer cards than a full set, so each one gets more
// room. The user's persisted size preference still drives it — here 'small' matches the
// catalog's 'medium' density, 'medium' matches the catalog's 'large', and 'large' goes bigger
// still (fewer columns than the catalog ever uses). Same literal-class rule as
// CARD_SIZE_GRID_CLASS: Tailwind's scanner only emits utilities it sees verbatim in the source.
export const DETAIL_CARD_SIZE_GRID_CLASS: Record<CardSize, string> = {
