...
// The CSV-upload tab's fields: the file picker plus the "how to export" hint naming the
// three columns we need. The parent owns the chosen file and the reconcile mode; this just
// emits the picked file (null when cleared). Rendered under v-if so the native file input
// remounts empty when the user leaves and returns to this tab.
...
<template>
  <div class="space-y-2">
    ...
    <div class="bg-muted/60 text-muted-foreground rounded-md p-3 text-xs">
      ...
      <p class="mt-1">
        In {{ providerLabel }}, open your collection and choose Export → CSV. You only need these
        three columns — you can leave the rest unchecked:
      </p>
