...
<template>
  <dl class="grid grid-cols-[8rem_1fr] gap-x-4 gap-y-2.5 text-sm">
    ...
    <!-- The Reserved List gets its own row rather than a "Printing" chip: it's a promise
         about the card's future, not a fact about this printing's ink. Neutral-but-notable,
         so the `info` token. -->
    <template v-if="card.reserved">
      <dt class="text-muted-foreground">Reserved List</dt>
      <dd>
        <span :class="[CHIP, 'bg-info/15 text-info']"> Reserved List — never to be reprinted </span>
      </dd>
    </template>
